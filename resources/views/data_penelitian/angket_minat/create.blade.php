            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('angket-minat.store') }}" method="POST">
                        ...
                        @for ($i = 1; $i <= 14; $i++)
                        <div class="mb-3">
                            <label for="nilai_{{ $i }}" class="form-label">Nilai {{ $i }}</label>
                            <input type="number" class="form-control" id="nilai_{{ $i }}" name="nilai_{{ $i }}" value="{{ old('nilai_'.$i) }}" min="1" max="5" required>
                        </div>
                        @endfor
